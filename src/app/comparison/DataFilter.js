      ...
      {filteredPolicies.length > 0 ? (
        filteredPolicies.map((policy) => (
          <div key={policy.policy}>
            ...
            <p>Min Insured Amount: {policy.min}</p>
            <p>Max Insured Amount: {policy.max}</p>
            <p>Max Term Years: {policy.maxYears(userAge)}</p>
          </div>
        ))
      ) : (
        <p>No policies available for your profile.</p>
      )}
    </div>
  );
}
